...
packages <- c("dplyr", "knitr", "kableExtra", "tidyverse", "tinytex", "magrittr" ,"readr" ,"geosphere", "RCurl", "plyr", "ggplot2", "tidyr", "stringr", "cowplot", "scales", "numform", "leaflet", "imputeTS")
...
lastday_confirmed <- 
  dplyr::select(confirmed, Province.State, Country.Region, Lat, Long, "Latest_Day"=tail(names(confirmed),1))

#variable holding dataframe with necessary columns for deaths
lastday_deaths <-
  dplyr::select(deaths, Province.State, Country.Region, Lat, Long, "Latest_Day"=tail(names(deaths),1))
...
#remove unnecessary data entries that involve Olympics or unknown origin
...
COVID_dataSum_country <- 
  dplyr::group_by(COVID_dataSum, Country.Region) %>%
...
#variable holding dataframe with necessary columns for confirmed
lastday_confirmed <- 
  dplyr::select(confirmed, Province.State, Country.Region, 
...
#variable holding dataframe with necessary columns for deaths
lastday_deaths <-
  dplyr::select(deaths, Province.State, Country.Region, 
...
COVID_Table %>%
  ...
  scroll_box(height = "550px", width = "800px")
...
```{r obj4-chart1, including=FALSE, echo=FALSE, eval=FALSE}
...
```{r obj4-chart2, echo=TRUE, eval=FALSE}
...
dfConfirmedCAChartData <- confirmed_US %>%
  ...
  # retains only the rows for San Diego, Riverside, Los Angeles
  filter(`Admin2` %in%
           c("San Diego", "Riverside", "Los Angeles")) %>%
...
USpop <- read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_US.csv")
...
Pop_confirmed <- left_join(Confirmed_needed, USpop)
